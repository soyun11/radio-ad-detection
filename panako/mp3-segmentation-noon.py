...
def split_mp3(input_file, output_dir, segment_length, step):
    ...
    while t + segment_length <= total_duration:
        ...
        if idx % 100 == 0:
            ...
            print(f"  [{idx:>4}/{total_clips}] {t:>7.1f}s / {total_duration:.1f}s  ({pct:.1f}%)", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Split noon radio MP3 into sliding-window clips"
    )
    parser.add_argument("date", type=str, help="Date (YYYYMMDD)")
    parser.add_argument(
        "--output_dir", type=str, default=None,
        help="Directory to save clips (default: clips_noon/<date>)"
    )
    parser.add_argument(
    ...
    output_dir = args.output_dir or join("clips_noon", date)
