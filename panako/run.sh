...
if [ -z "$BROADCASTER" ]; then
    echo "Usage: bash run.sh <broadcaster>"
    echo "       broadcaster: baechulsu | movie | noon"
    exit 1
fi
...
case $BROADCASTER in
    baechulsu)
        BASE_DIR="baechulsu"
        DATES=(20241124 20241125 20241126 20260101 20260102 20260103 20260104 20260105 20260106 20260107)
        LABEL="MBC 배철수의 음악캠프"
        ;;
    movie)
        BASE_DIR="movie"
        DATES=(20241120 20241121 20241122 20241123 20241124 20241125 20241126 20241127 20241128 20241129)
        LABEL="SBS 박하선의 씨네타운"
        ;;
    noon)
        BASE_DIR="noon"
        ...
        ;;
    *)
        echo "❌ 알 수 없는 방송사: $BROADCASTER"
        echo "   사용 가능: baechulsu | movie | noon"
        exit 1
        ;;
esac

WORK_DIR="panako/$BROADCASTER"
mkdir -p $WORK_DIR/clips
mkdir -p $WORK_DIR/results

echo "========================================"
echo "🚀 $LABEL Panako 광고 탐지 (20초 클립)"
echo "========================================"
echo "날짜: ${DATES[*]}"
echo "작업 폴더: $WORK_DIR"
echo ""
...
for DATE in "${DATES[@]}"; do
    ...
    if [ -f "$MP3_PATH" ]; then
        python mp3_segmentation.py $BROADCASTER $DATE --output_dir $WORK_DIR/clips/$DATE --segment_length 20
    else
